...
const ContentCardGrid: React.FC<ContentCardGridProps> = ({ item, selectedCountries }) => {
  ...
  return (
    <div className="group relative bg-slate-900/40 backdrop-blur-xl rounded-[2rem] overflow-hidden border border-slate-800/60 hover:border-indigo-500/50 transition-all duration-500 flex flex-col shadow-2xl hover:shadow-indigo-500/10">
      <div className="w-full h-64 flex-shrink-0 relative overflow-hidden">
        ...
        <div className="absolute top-4 left-4">
         ...
         <div className="absolute flex flex-row right-4 bottom-4 gap-2 px-4 py-2 bg-slate-950/80 rounded-2xl border border-slate-800/50 text-yellow-500 flex-shrink-0">
            <Star size={18} fill="currentColor" className="self-center" />
            <span className="text-md font-black">{item.rating}</span>
          </div>
      </div>
        ...
        <div>
          <div className="flex justify-between items-start gap-6 mb-4">
            <div className="flex flex-col space-y-1 w-full">
              <h4 className="text-xl sm:text-3xl font-black text-white leading-tight group-hover:text-indigo-400 transition-colors">
                {item.title}
              </h4>
              <div className="flex items-center gap-3 text-slate-500 text-xs font-bold uppercase tracking-widest">
                ...
                <div className="flex gap-2 items-center">
                  {item.genre.map((label, idx) => (
                    <span key={idx} className="text-indigo-400/80 text-[10px]">
